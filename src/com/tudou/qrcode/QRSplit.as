	...
	public class QRSplit
	{
		...
		public function eatNum():int
		{
			...
			if(mode == QRCodeEncodeType.QRCODE_ENCODE_BYTES) {
				var i:int = QRInput.estimateBitsModeNum(run) + 4 + ln
		...
		public function eatAn():int
		{
			...
			if(!isalnumat(this.dataStr, p)) {
				var dif2:int = QRInput.estimateBitsModeAn(run) + 4 + la
					+ QRInput.estimateBitsMode8(1) // + 4 + l8
					- QRInput.estimateBitsMode8(run + 1); // - 4 - l8
				if(dif2 > 0) {
					return this.eat8();
		...
		public function eat8():int
		{
			...
			while(p < dataStrLen) {
				...
				if(mode == QRCodeEncodeType.QRCODE_ENCODE_NUMERIC) {
					...
					var q2:int = p;
					while(isalnumat(this.dataStr, q2)) {
						q2++;
					}
					var dif2:int = QRInput.estimateBitsMode8(p)  // + 4 + l8
						+ QRInput.estimateBitsModeAn(q2 - p) + 4 + la
						- QRInput.estimateBitsMode8(q2); // - 4 - l8
					if(dif2 < 0) {
						break;
					} else {
						p = q2;
